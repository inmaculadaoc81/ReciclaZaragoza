...
module.exports=async function(req,res){
 ...
 if(req.method==="GET")return res.status(200).json({ok:true,service:"ReciclaZaragoza contacto API",node:process.version,environment:Object.fromEntries(required.map(k=>[k,!!process.env[k]]))});
 ...
 try{
  ...
  const gmail=google.gmail({version:"v1",auth});const subject="Nueva consulta ReciclaZaragoza";
  const html=`<h2>Nueva consulta ReciclaZaragoza</h2><p><b>Nombre:</b> ${n}</p><p><b>Teléfono:</b> ${p}</p><p><b>Email:</b> ${e}</p><p><b>Equipos / cantidad:</b> ${d||"No indicado"}</p><p><b>Consulta:</b><br>${m.replace(/\n/g,"<br>")}</p>`;
  const raw=[`From: ReciclaZaragoza <${process.env.GOOGLE_EMAIL}>`,`To: ${process.env.CONTACT_EMAIL}`,`Reply-To: ${e}`,`Subject: =?UTF-8?B?${Buffer.from(subject).toString("base64")}?=`,"MIME-Version: 1.0","Content-Type: text/html; charset=UTF-8","",html].join("\r\n");
  await gmail.users.messages.send({userId:"me",requestBody:{raw:Buffer.from(raw).toString("base64url")}});
  return res.status(200).json({ok:true});
 }catch(error){console.error("ReciclaZaragoza Gmail API error:",error);return res.status(500).json({ok:false,code:"EMAIL_SEND_FAILED"});}
};